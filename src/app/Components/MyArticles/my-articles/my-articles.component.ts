...
import { ArticleSkeletonComponent } from '../../Articles/Skeleton/article-skeleton.component';
...
export class MyArticlesComponent implements OnInit, OnDestroy {
  userArticleService = inject(UserArticleService);
  ...
  constructor() {
    effect(() => {
      if (this.navService.showCreateModal()) {
  ...
  loadMore(): void {
    ...
    this.userArticleService
      .getUserArticles(nextPage, this.pageSize(), this.searchTerm())
  ...
  private initializeArticles(): void {
    ...
    this.userArticleService
      .getUserArticles(this.currentPage(), this.pageSize(), this.searchTerm())
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (response) => {
