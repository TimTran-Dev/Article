...
@Component({
  ...
  template: ` <!-- episode.component.html -->

    @if (isLoading()) {
      <tap-loading loadingText="Loading episodes..."></tap-loading>
    }
    ...
    @if (!isLoading()) {
      <div class="min-h-screen bg-gray-50">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <!-- Header Section -->
          <div class="mb-8">
            <h1 class="text-4xl font-bold text-gray-900 mb-2">Episodes</h1>
            <p class="text-gray-600 text-lg">Manage and review your episode content</p>
          </div>
          ...
          >
            ...
            <span class="text-sm text-gray-500">{{ episode().length }} episodes</span>
          </div>

          <!-- Episode List -->
          <div class="space-y-4">
            @for (product of episode(); track product.id) {
              @if ('airDate' in product.content) {
                <article
                ...
                >
                  <div class="p-6">
                    ...
                    <!-- Episode Preview/Body (truncated) -->
          ...
          @if (episode().length === 0) {
            <div class="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <p class="text-gray-500 mb-6">Start by creating your first episode.</p>
              <button
...
export class EpisodeComponent implements OnInit, OnDestroy {
  episode = signal<(Content & { contentStatusSignal: WritableSignal<ContentStatus> })[]>([]);
  ...
  private initializeEpisodes(): void {
    ...
    this.productService
      ...
      .pipe(
        ...
        map((items) => items.filter((item) => item.contentType === 'Episode')),
      )
      .subscribe({
        next: (episodeItems) => {
          const reactiveEpisodes = episodeItems.map((content) => ({
            ...content,
